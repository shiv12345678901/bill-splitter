...
export function calculateTransfers(members: SettlementMember[], expenses: SettlementExpense[]): Transfer[] {
  ...
  for (const expense of expenses) paid.set(expense.payerId, (paid.get(expense.payerId) ?? 0) + Math.round(expense.amount * 100));

  const balances = members.map((member, index) => ({
    ...member,
    cents: (paid.get(member.id) ?? 0) - baseShare - (index < remainder ? 1 : 0),
  }));
  const debtors = balances.filter((member) => member.cents < 0).map((member) => ({ ...member, cents: -member.cents }));
  const creditors = balances.filter((member) => member.cents > 0).map((member) => ({ ...member }));
  ...
  while (debtorIndex < debtors.length && creditorIndex < creditors.length) {
    ...
    if (cents > 0) transfers.push({ fromId: debtor.id, from: debtor.name, toId: creditor.id, to: creditor.name, amount: cents / 100 });
    debtor.cents -= cents;
