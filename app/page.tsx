...
import { ArrowRight, Camera, ChartNoAxesColumnIncreasing, Check, ChevronRight, Download, Fingerprint, Home, ImagePlus, Link2, LogOut, Mail, Palette, Pencil, Plus, ReceiptText, RefreshCw, Search, Settings2, Share2, ShieldCheck, Trash2, UserMinus, Users, WifiOff, X } from 'lucide-react';
...
type Member = { id: string; household_id: string; user_id: string; name: string; color: string; role: 'admin' | 'user' };
type Expense = { id: string; householdId: string; payerId: string; payerName: string; merchant: string; amount: number; category: string; receiptDate: string; createdAt: string; image?: string; imagePath?: string; settled: boolean; ocrStatus: string; ocrModel?: string; notes?: string; syncStatus?: 'pending' | 'failed' };
...
type ReceiptFlag = { id: string; expense_id: string; reported_by: string; reason: string | null; status: 'open' | 'resolved'; created_at: string };
...
export default function HomePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!online) { setError('Connect once to load this household on this iPhone.'); setDataLoading(false); return; }
    const [memberResult, expenseResult, cycleResult, activityResult, categoryResult, flagResult] = await Promise.all([
      ...
      supabase.from('receipt_flags').select('id,expense_id,reported_by,reason,status,created_at').eq('household_id', household.id).eq('status', 'open').order('created_at', { ascending: false }),
    ]);
  ...
  const syncOfflineReceipts = useCallback(async () => {
    ...
    for (const receipt of queued) {
      ...
      if (receipt.file) {
        ...
        if (upload.error && !upload.error.message.toLowerCase().includes('exist')) { failed += 1; await saveOfflineReceipt({ ...receipt, attempts: (receipt.attempts ?? 0) + 1, syncError: upload.error.message }); continue; }
      }
      ...
      if (result.error && result.error.code !== '23505') { if (imagePath) await supabase.storage.from('receipts').remove([imagePath]); failed += 1; await saveOfflineReceipt({ ...receipt, attempts: (receipt.attempts ?? 0) + 1, syncError: result.error.message }); continue; }
    ...
    if (failed) { setExpenses((current) => current.map((expense) => queued.some((receipt) => receipt.id === expense.id) ? { ...expense, syncStatus: 'failed' } : expense)); notify(`${failed} receipt${failed === 1 ? '' : 's'} could not sync · tap Retry`); }
  ...
  useEffect(() => {
    ...
    void supabase.auth.passkey.list().then(({ data, error: passkeyError }) => {
      if (active && !passkeyError) setPasskeyConfigured((data?.length ?? 0) > 0);
    });
    return () => { active = false; };
  }, [user]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator) void navigator.serviceWorker.register('/sw.js').then((value) => { registration = value; }).catch((serviceWorkerError) => console.error('[SplitMate] service worker registration failed', serviceWorkerError));
    return () => { window.removeEventListener('online', goOnline); window.removeEventListener('offline', goOffline); document.removeEventListener('visibilitychange', checkForUpdate); };
  }, []);
  ...
  useEffect(() => {
    if (!household) return;
    const channel = supabase.channel(`household:${household.id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'expenses', filter: `household_id=eq.${household.id}` }, () => setRefreshKey((value) => value + 1)).on('postgres_changes', { event: '*', schema: 'public', table: 'household_members', filter: `household_id=eq.${household.id}` }, () => setRefreshKey((value) => value + 1)).on('postgres_changes', { event: '*', schema: 'public', table: 'settlement_cycles', filter: `household_id=eq.${household.id}` }, () => setRefreshKey((value) => value + 1)).on('postgres_changes', { event: '*', schema: 'public', table: 'household_activity', filter: `household_id=eq.${household.id}` }, () => setRefreshKey((value) => value + 1)).on('postgres_changes', { event: '*', schema: 'public', table: 'household_categories', filter: `household_id=eq.${household.id}` }, () => setRefreshKey((value) => value + 1)).on('postgres_changes', { event: '*', schema: 'public', table: 'receipt_flags', filter: `household_id=eq.${household.id}` }, () => setRefreshKey((value) => value + 1)).subscribe();
  ...
  const receiptReadOnly = Boolean(detail && (detail.settled || !isAdmin));
  const detailFlags = detail ? receiptFlags.filter((flag) => flag.expense_id === detail.id) : [];
  ...
  const scanFile = async (file: File, existingPath = '') => {
    ...
    try {
      ...
      if (invoked.error || invoked.data?.error) throw invoked.error ?? new Error(invoked.data.error);
  ...
  const openDetail = (expense: Expense) => { if (expense.syncStatus) { notify(expense.syncStatus === 'failed' ? 'Sync failed · reconnect and tap Retry' : 'This receipt will be available after it syncs'); return; } setDetail(expense); setFlagReason(''); setDraft({ ...blankDraft(), merchant: expense.merchant, amount: String(expense.amount), category: expense.category, receiptDate: expense.receiptDate, payerId: expense.payerId, image: expense.image ?? '', imagePath: expense.imagePath ?? '', notes: expense.notes ?? '', ocrStatus: expense.ocrStatus }); setSheetExpanded(false); setSheetOpen(true); };
  const saveExpense = async () => {
    const amount = Number(draft.amount);
    if (!user || !household || !draft.payerId || !draft.merchant.trim() || !(amount > 0) || (detail && !isAdmin)) return;
  ...
  const deleteExpense = async (expense: Expense) => {
    if (!isAdmin && expense.syncStatus !== 'pending') { notify('Only an admin can delete receipts'); return; }
  ...
  const settleCycle = async () => { if (!isAdmin || !household || !activeExpenses.length) return; setSaving(true); const result = await supabase.rpc('settle_household', { target_household: household.id, transfer_summary: transfers }); setSaving(false); if (result.error) notify(result.error.message); else { setSettlementReview(false); setRefreshKey((value) => value + 1); notify('Cycle settled'); } };
  const shareSettlement = async () => { if (!isAdmin) return; const text = `${household?.name ?? 'Household'} settlement\n${transfers.length ? transfers.map((transfer) => `${transfer.from} pays ${transfer.to} ${money.format(transfer.amount)}`).join('\n') : 'Everyone is even.'}`; if (navigator.share) await navigator.share({ title: 'SplitMate settlement', text }); else { await navigator.clipboard.writeText(text); notify('Settlement copied'); } };
  const registerPasskey = async () => {
    if (passkeyConfigured) { notify('A passkey is already set up'); return; }
    setPasskeyBusy(true);
    try {
      const result = await supabase.auth.registerPasskey();
      if (result.error) {
        if (result.error.code === 'webauthn_credential_exists') { setPasskeyConfigured(true); notify('This passkey is already set up'); }
        else notify(result.error.message);
      } else { setPasskeyConfigured(true); notify('Face ID or passkey is ready'); }
  ...
  const exportExpenses = () => {
    ...
    const rows = [['Date', 'Merchant', 'Amount AUD', 'Category', 'Paid by', 'Status', 'Notes'], ...expenses.map((expense) => [expense.receiptDate, expense.merchant, expense.amount.toFixed(2), expense.category, expense.payerName, expense.settled ? 'Settled' : 'Current', expense.notes ?? ''])];
    const blob = new Blob([rows.map((row) => row.map(cell).join(',')).join('\r\n')], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `splitmate-${household?.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'household'}-${localDate()}.csv`; link.click(); URL.revokeObjectURL(url); notify('Expense export downloaded');
  };
  const deleteAccount = async () => {
    if (!window.confirm('Permanently delete your SplitMate account? Shared household records will be preserved when other members remain. This cannot be undone.')) return;
    setDeletingAccount(true);
    const result = await supabase.functions.invoke('delete-account', { body: { confirm: true } });
    if (result.error || result.data?.error) { notify(result.data?.error ?? result.error?.message ?? 'Account could not be deleted'); setDeletingAccount(false); return; }
    window.localStorage.clear(); await supabase.auth.signOut();
  };
  ...
  return <main className="app-shell">
    ...
    <div className="app-frame"><header className="topbar"><div><p>{household.name}</p><h1>{tab === 'home' ? greeting : tab === 'receipts' ? 'Receipts' : tab === 'insights' ? 'Insights' : tab === 'household' ? 'Household' : 'Settings'}</h1></div>{(tab === 'home' || tab === 'receipts') && <div className="top-actions"><button onClick={openCamera} className="circle-button secondary" aria-label="Scan with camera"><Camera size={20} /></button><button onClick={openUpload} className="circle-button" aria-label="Choose bill from Photos"><ImagePlus size={21} /></button></div>}</header>{error && <div className="error-banner"><span>{error}</span><button onClick={() => setRefreshKey((value) => value + 1)}>Retry</button></div>}
      {tab === 'home' && <div className="content-grid"><section className="stack"><div className="balance-card"><p>{cycleLabel}</p><strong>{money.format(total)}</strong><span>{money.format(share)} each · {activeExpenses.length} receipts</span><div className="member-stack">{members.map((member) => <Avatar key={member.id} member={member} small />)}</div></div><div className="source-actions"><button className="capture-button" onClick={openUpload}><ImagePlus size={22} /><span><strong>Choose bill from Photos</strong><small>{online ? 'Scanning image fills in the details' : 'Save now and sync when online'}</small></span><ChevronRight size={20} /></button></div>{expenses.some((expense) => expense.syncStatus === 'failed') && <div className="attention-card"><WifiOff size={19} /><div><strong>Receipt sync needs attention</strong><span>Your bill is safe on this iPhone.</span></div><button disabled={!online} onClick={() => void syncOfflineReceipts()}>Retry</button></div>}<section><div className="section-heading"><h2>Recent receipts</h2><button onClick={() => setTab('receipts')}>See all</button></div><div className="list-card">{activeExpenses.length ? activeExpenses.slice(0, 3).map(receiptRow) : <div className="empty-state"><ReceiptText /><strong>No bills this cycle</strong><p>Choose one from Photos to get started.</p></div>}</div></section></section><section className="stack"><div className="section-heading"><h2>Settle up</h2><span>{members.length} people</span></div><div className="settlement-card">{!activeExpenses.length ? <div className="empty-state compact"><Check /><strong>Everything is settled</strong></div> : transfers.length ? transfers.map((transfer) => <div className="transfer-row" key={`${transfer.fromId}-${transfer.toId}`}><span><strong>{transfer.from}</strong> pays {transfer.to}</span><b>{money.format(transfer.amount)}</b></div>) : <div className="empty-state compact"><Check /><strong>Everyone is even</strong></div>}{isAdmin ? <div className="settlement-actions"><button disabled={!activeExpenses.length || saving} onClick={() => void shareSettlement()}><Share2 size={18} /> Share</button><button disabled={!activeExpenses.length || saving} onClick={() => setSettlementReview(true)}><Check size={18} /> Review settlement</button></div> : <p className="permission-note"><ShieldCheck size={16} /> Only admins can share or settle this cycle.</p>}</div></section></div>}
      {tab === 'receipts' && <section className="single-column"><div className="search-field"><Search size={18} /><input aria-label="Search receipts" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search merchant, category or person" />{search && <button className="search-clear" onClick={() => setSearch('')} aria-label="Clear search"><X size={14} /></button>}</div><div className="segmented"><button aria-pressed={receiptFilter === 'active'} className={receiptFilter === 'active' ? 'selected' : ''} onClick={() => setReceiptFilter('active')}>Current · {expenses.filter((expense) => !expense.settled).length}</button><button aria-pressed={receiptFilter === 'settled'} className={receiptFilter === 'settled' ? 'selected' : ''} onClick={() => setReceiptFilter('settled')}>History · {expenses.filter((expense) => expense.settled).length}</button></div><div className="list-card">{filteredExpenses.length ? filteredExpenses.map(receiptRow) : <div className="empty-state"><Search /><strong>No matching receipts</strong><p>Try a different search.</p></div>}</div></section>}
      {tab === 'insights' && <Suspense fallback={<section className="single-column insights-page"><div className="insights-loading" aria-label="Loading insights" /></section>}><InsightsView expenses={expenses} members={members} cycles={cycles} /></Suspense>}
      {tab === 'household' && <section className="single-column household-page">{isAdmin ? <div className="invite-card invite-expanded"><div><p>Invite to household</p><strong>{household.join_code}</strong><span>Share a secure link or let someone scan the code.</span><div className="invite-actions"><button onClick={() => void shareInvite()}><Link2 size={17} /> Share invite</button><button onClick={() => void regenerateInvite()}><RefreshCw size={16} /> Replace</button></div></div>{qrCode && <img src={qrCode} alt="Household invitation QR code" />}</div> : <div className="permission-card"><ShieldCheck size={20} /><div><strong>Invites are managed by admins</strong><span>Ask an admin to invite someone new.</span></div></div>}<div className="section-heading"><h2>Members</h2><span>Every bill is split equally</span></div><div className="list-card">{members.map((member) => <div className="member-row managed" key={member.id}><Avatar member={member} /><div><strong>{member.name} <span className={`role-pill ${member.role}`}>{member.role}</span></strong><span>{member.user_id === self.user_id ? 'You' : 'Household member'}</span></div>{isAdmin && <div className="member-controls"><select aria-label={`${member.name} role`} value={member.role} onChange={(event) => void changeMemberRole(member, event.target.value as Member['role'])}><option value="user">User</option><option value="admin">Admin</option></select>{member.user_id !== user.id && <button onClick={() => void removeMember(member)} aria-label={`Remove ${member.name}`}><UserMinus size={18} /></button>}</div>}</div>)}</div>{activities.length > 0 && <div><div className="section-heading"><h2>Activity</h2><span>Latest changes</span></div><div className="list-card activity-list">{activities.slice(0, 12).map((activity) => <div className="activity-row" key={activity.id}><span className={`activity-dot ${activity.action}`} /><div><strong>{activity.actor_name || 'SplitMate'} {activity.summary}</strong><small>{new Date(activity.created_at).toLocaleString('en-AU', { day: 'numeric', month: 'short', hour: 'numeric', minute: '2-digit' })}</small></div></div>)}</div></div>}</section>}
      {tab === 'settings' && <section className="single-column settings-page"><div className="settings-profile"><Avatar member={self} /><div><strong>{self.name} <span className={`role-pill ${self.role}`}>{self.role}</span></strong><span>{user.email}</span></div></div><div><p className="settings-label">Profile</p><div className="settings-group"><label className="inline-setting"><span className="settings-icon"><Pencil size={18} /></span><input value={profileName} onChange={(event) => setProfileName(event.target.value)} maxLength={50} aria-label="Display name" /><button disabled={!profileName.trim() || profileName.trim() === self.name} onClick={() => void saveProfileName()}>Save</button></label>{isAdmin && <label className="inline-setting"><span className="settings-icon"><Home size={18} /></span><input value={householdName} onChange={(event) => setHouseholdName(event.target.value)} maxLength={60} aria-label="Household name" /><button disabled={!householdName.trim() || householdName.trim() === household.name} onClick={() => void saveHouseholdName()}>Save</button></label>}</div></div>{isAdmin && <div><p className="settings-label">Categories</p><div className="settings-group"><label className="inline-setting"><span className="settings-icon"><Plus size={19} /></span><input value={newCategory} onChange={(event) => setNewCategory(event.target.value)} maxLength={30} placeholder="New category" aria-label="New category" /><button disabled={!newCategory.trim()} onClick={() => void addCategory()}>Add</button></label>{customCategories.map((category) => <div className="settings-row compact-setting" key={category.id}><span>{category.name}</span><button aria-label={`Delete ${category.name}`} onClick={async () => { const result = await supabase.from('household_categories').delete().eq('id', category.id); if (result.error) notify(result.error.message); else setRefreshKey((value) => value + 1); }}><X size={16} /></button></div>)}</div></div>}<div><p className="settings-label">Appearance</p><div className="settings-group"><div className="settings-row"><span className="settings-icon"><Palette size={19} /></span><div><strong>Appearance</strong><small>Choose how SplitMate looks.</small></div><div className="theme-picker" aria-label="Appearance">{(['system', 'light', 'dark'] as const).map((value) => <button key={value} className={theme === value ? 'selected' : ''} onClick={() => setTheme(value)}>{value[0].toUpperCase() + value.slice(1)}</button>)}</div></div></div></div><div><p className="settings-label">Security</p><div className="settings-group">{typeof PublicKeyCredential !== 'undefined' && (passkeyConfigured ? <div className="settings-row passkey-ready"><span className="settings-icon"><Check size={20} /></span><div><strong>Passkey is set up</strong><small>Face ID or your device passkey can sign you in.</small></div></div> : <button className="settings-row settings-button" disabled={passkeyBusy} onClick={() => void registerPasskey()}><span className="settings-icon"><Fingerprint size={20} /></span><div><strong>{passkeyBusy ? 'Opening security check…' : 'Set up Face ID or passkey'}</strong><small>Sign in without entering your password.</small></div><ChevronRight size={18} /></button>)}<div className="settings-row"><span className="settings-icon"><ShieldCheck size={19} /></span><div><strong>Private household</strong><small>Only members can see receipts and balances.</small></div></div></div></div><div><p className="settings-label">Data & privacy</p><div className="settings-group"><button className="settings-row settings-button" onClick={exportExpenses}><span className="settings-icon"><Download size={19} /></span><div><strong>Export expenses</strong><small>Download a portable CSV backup.</small></div><ChevronRight size={18} /></button><Link className="settings-row settings-button settings-link" href="/privacy"><span className="settings-icon"><ShieldCheck size={19} /></span><div><strong>Privacy & receipt scanning</strong><small>See what is stored and sent for OCR.</small></div><ChevronRight size={18} /></Link></div></div><div><p className="settings-label">Account</p><div className="settings-group"><button className="settings-row settings-button danger-row" onClick={() => void leaveHousehold()}><span className="settings-icon"><UserMinus size={19} /></span><div><strong>{isOwner ? 'Delete household' : 'Leave household'}</strong><small>{isOwner ? 'Only possible when no other members remain.' : 'Remove your account from this household.'}</small></div><ChevronRight size={18} /></button><button className="settings-row settings-button danger-row" disabled={deletingAccount} onClick={() => void deleteAccount()}><span className="settings-icon"><Trash2 size={19} /></span><div><strong>{deletingAccount ? 'Deleting account…' : 'Permanently delete account'}</strong><small>Deletes your login and personal account data.</small></div><ChevronRight size={18} /></button><button className="settings-row settings-button danger-row" onClick={() => void supabase.auth.signOut()}><span className="settings-icon"><LogOut size={19} /></span><div><strong>Sign out</strong><small>Keep this device’s data private.</small></div><ChevronRight size={18} /></button></div></div><p className="settings-footer">SplitMate · Household bills, sorted.</p></section>}
      {settlementReview && <div className="confirm-backdrop" role="presentation"><div className="confirm-card settlement-review" role="alertdialog" aria-modal="true" aria-labelledby="settlement-title"><span className="confirm-icon settle-icon"><Check size={24} /></span><h2 id="settlement-title">Review settlement</h2><p>This closes {activeExpenses.length} receipt{activeExpenses.length === 1 ? '' : 's'} totalling {money.format(total)} for everyone.</p><div className="settlement-review-list">{transfers.length ? transfers.map((transfer) => <span key={`${transfer.fromId}-${transfer.toId}`}><b>{transfer.from}</b> pays {transfer.to}<strong>{money.format(transfer.amount)}</strong></span>) : <span><b>Everyone is even</b><strong>{money.format(0)}</strong></span>}</div><div className="confirm-actions"><button disabled={saving} onClick={() => setSettlementReview(false)}>Go back</button><button className="settle-confirm" disabled={saving} onClick={() => void settleCycle()}>{saving ? 'Settling…' : 'Confirm settlement'}</button></div></div></div>}
    </div>
    <nav className="bottom-nav" aria-label="Primary navigation"><button aria-current={tab === 'home' ? 'page' : undefined} className={tab === 'home' ? 'active' : ''} onClick={() => setTab('home')}><Home size={21} /><span>Home</span></button><button aria-current={tab === 'receipts' ? 'page' : undefined} className={tab === 'receipts' ? 'active' : ''} onClick={() => setTab('receipts')}><ReceiptText size={21} /><span>Receipts</span></button><button aria-current={tab === 'insights' ? 'page' : undefined} className={tab === 'insights' ? 'active' : ''} onClick={() => setTab('insights')}><ChartNoAxesColumnIncreasing size={21} /><span>Insights</span></button><button aria-current={tab === 'household' ? 'page' : undefined} className={tab === 'household' ? 'active' : ''} onClick={() => setTab('household')}><Users size={21} /><span>Household</span></button><button aria-current={tab === 'settings' ? 'page' : undefined} className={tab === 'settings' ? 'active' : ''} onClick={() => setTab('settings')}><Settings2 size={21} /><span>Settings</span></button></nav>
    {sheetOpen && <div className="sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) closeEditor(); }}><dialog open className={`receipt-sheet ${sheetExpanded ? 'sheet-expanded' : ''}`} aria-labelledby="receipt-title"><button className="sheet-handle" aria-label={sheetExpanded ? 'Make receipt panel smaller' : 'Expand receipt panel'} onClick={() => setSheetExpanded((value) => !value)} onTouchStart={(event) => { sheetTouchStart.current = event.touches[0].clientY; }} onTouchEnd={(event) => { const distance = event.changedTouches[0].clientY - sheetTouchStart.current; if (distance < -24) setSheetExpanded(true); if (distance > 24) setSheetExpanded(false); }}><span className="grabber" /></button><header><button className="sheet-close" onClick={() => closeEditor()} disabled={saving}><X /></button><div><p>{detail ? 'Receipt details' : analyzing ? 'Scanning image' : 'Review details'}</p><h2 id="receipt-title">{detail?.settled ? 'Settled receipt' : detail && isAdmin ? 'Edit receipt' : detail ? 'Receipt details' : 'New receipt'}</h2></div>{detail && !detail.settled && isAdmin ? <button className="sheet-delete" onClick={() => requestDelete(detail)}><Trash2 size={18} /> Delete</button> : <span />}</header>{draft.image && <img className="receipt-preview" src={draft.image} alt="Selected receipt" />}{analyzing && <div className="scan-status"><span className="loader small" />Scanning image securely…</div>}{!online && !detail && <div className="scan-status offline-status"><WifiOff size={16} />Saved on this iPhone until you reconnect</div>}{draft.lowConfidence && <div className="warning-banner">Some details may be unclear. Please check the highlighted result.</div>}{draft.duplicate && <div className="warning-banner">This looks like a receipt already saved.</div>}{detailFlags.map((flag) => <div className="flag-banner" key={flag.id}><div><strong>Marked as incorrect</strong><span>{flag.reason || 'No details were added.'}</span></div>{isAdmin && <button onClick={() => void resolveFlag(flag)}>Resolve</button>}</div>)}<div className="editor-fields"><label className="field-label">Merchant<input disabled={receiptReadOnly} className="field-input" value={draft.merchant} onChange={(event) => setDraft({ ...draft, merchant: event.target.value })} /></label><div className="two-columns"><label className="field-label">Total (AUD)<input disabled={receiptReadOnly} className="field-input" inputMode="decimal" value={draft.amount} onChange={(event) => setDraft({ ...draft, amount: event.target.value })} /></label><label className="field-label">Category<select disabled={receiptReadOnly} className="field-input" value={draft.category} onChange={(event) => setDraft({ ...draft, category: event.target.value })}>{allCategories.map((category) => <option key={category}>{category}</option>)}</select></label></div><div className="two-columns"><label className="field-label">Receipt date<input disabled={receiptReadOnly} className="field-input" type="date" value={draft.receiptDate} onChange={(event) => setDraft({ ...draft, receiptDate: event.target.value })} /></label><label className="field-label">Paid by<select disabled={receiptReadOnly} className="field-input" value={draft.payerId} onChange={(event) => setDraft({ ...draft, payerId: event.target.value })}>{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label></div><label className="field-label">Note <span className="optional">Optional</span><textarea disabled={receiptReadOnly} className="field-input field-textarea" maxLength={300} value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} placeholder="What was this bill for?" /></label><p className="privacy-note">Scanning sends this image securely to the receipt-reading service. All household members share the expense equally.</p>{detail && !isAdmin ? <div className="incorrect-report"><label className="field-label">Found a mistake? <span className="optional">Optional detail</span><textarea className="field-input field-textarea" maxLength={300} value={flagReason} onChange={(event) => setFlagReason(event.target.value)} placeholder="Tell an admin what looks wrong" /></label><button disabled={detailFlags.some((flag) => flag.reported_by === user.id)} onClick={() => void reportIncorrect()}>{detailFlags.some((flag) => flag.reported_by === user.id) ? 'Already marked for review' : 'Mark as incorrect'}</button></div> : detail?.settled ? <div className="settled-note"><Check size={17} /> Included in a completed settlement</div> : <>{online && isAdmin && (draft.file || detail?.imagePath) && <button className="rescan-button" disabled={analyzing || saving} onClick={() => void rescanDetail()}><RefreshCw size={17} /> Scan image again</button>}<button className="primary-button" disabled={analyzing || saving || !draft.payerId || !draft.merchant.trim() || !(Number(draft.amount) > 0)} onClick={() => void saveExpense()}>{saving ? 'Saving…' : !online && !detail ? 'Save offline' : detail ? 'Save changes' : 'Add receipt'} {!saving && <ArrowRight size={18} />}</button></>}</div></dialog></div>}
    ...
    {toast && <div className="toast" role="status" aria-live="polite"><Check size={16} /><span>{toast.message}</span>{toast.action && <button onClick={toast.action.run}>{toast.action.label}</button>}</div>}
  </main>;
}
