...
export default function HomePage() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      appFrameRef.current?.scrollTo({ top: 0, left: 0, behavior: 'instant' });
  ...
  useEffect(() => {
    ...
    const frameOverflow = appFrame?.style.overflow;
    ...
    if (appFrame) appFrame.style.overflow = 'hidden';
    Object.assign(document.body.style, { overflow: 'hidden', position: 'fixed', top: `-${scrollY}px`, width: '100%' });
    return () => { if (appFrame) appFrame.style.overflow = frameOverflow ?? ''; Object.assign(document.body.style, previous); window.scrollTo(0, scrollY); };
