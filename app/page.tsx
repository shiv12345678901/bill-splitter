...
import { ArrowRight, Camera, Check, ChevronRight, Copy, Fingerprint, Home, ImagePlus, LogOut, Mail, Palette, ReceiptText, RefreshCw, Search, Settings2, Share2, ShieldCheck, Trash2, Users, WifiOff, X } from 'lucide-react';
...
export default function HomePage() {
  ...
  const [tab, setTab] = useState<'home' | 'receipts' | 'household' | 'settings'>('home');
  ...
  const closeEditor = () => { if (draft.image.startsWith('blob:')) URL.revokeObjectURL(draft.image); setSheetOpen(false); setSheetExpanded(false); setDetail(null); setDraft(blankDraft()); setSwipedId(null); };
  ...
  const handleFile = async (original?: File) => {
    ...
    try {
      ...
      setSheetExpanded(false); setSheetOpen(true); setDetail(null); await scanFile(file);
  ...
  const openDetail = (expense: Expense) => { setDetail(expense); setDraft({ ...blankDraft(), merchant: expense.merchant, amount: String(expense.amount), category: expense.category, receiptDate: expense.receiptDate, payerId: expense.payerId, image: expense.image ?? '', ocrStatus: expense.ocrStatus }); setSheetExpanded(false); setSheetOpen(true); };
